...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM fully loaded, initializing display audio');
    initializeDisplayAudio();
});

function initializeDisplayAudio() {
    console.log('initializeDisplayAudio called');
    ...
    // Reduced polling to 1 second for faster updates
    ...
    function fetchUpdates() {
        ...
        console.log('Fetching updates from', url.toString());
        fetch(url, {
            headers: {
            ...
            .then(data => {
                ...
                console.log('Calling updateMainToken with', data.mainToken);
    ...
    function updateMainToken(token) {
        ...
        if (!token) {
            ...
            `;
            console.log('Displayed waiting state');
            return;
        }
        ...
        container.innerHTML = `
            <div id="main-token-card"
     class="bg-gradient-to-r from-indigo-300 via-purple-300 to-pink-300
            rounded-3xl shadow-2xl w-full h-full flex items-center
            justify-around px-16 border-2 border-indigo-200 pulse-animation">
        ...
        `;
        console.log('Updated main token display to', token.token_number);
    }
    ...
    function showTokenModal(token) {
        ...
        console.log('Showing token modal for', token.token_number);
    }
    ...
    function hideTokenModal() {
        ...
        console.log('Hiding token modal');
    }
        ...
        langs.forEach(lang => {
            chain = chain.then(() => {
                return new Promise(resolve => {
                    ...
                    playPlaylist(langPlaylist, resolve);
                });
            });
        });
        chain.then(() => {
    ...
    function buildPlaylistForLang(lang, token, counter) {
        ...
        tokenParts.forEach(part => {
            if (isNaN(part)) {
                ...
                const files = lang === 'ar' ? getArabicNumberFiles(num) : getEnglishNumberFiles(num);
